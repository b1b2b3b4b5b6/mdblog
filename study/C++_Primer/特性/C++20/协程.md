...
```
无论是`co_return/co_yield`，只是单纯触发调用promise的方法，具体怎么将值传递出去还是依赖用户实现（比如将value保存到成员变量，然后提供方法获取）
### 编译器获取awaiter的伪代码
```C++
...
- 调用方获得的返回值对象由`promise_type`的`get_return_object`创建返回
- 返回值对象内可以不包含任何成员变量，一般会保存协程句柄，在`get_return_object`创建时赋值
...
- 协程对外体现是一个`协程函数`, `挂起`和`恢复`行为也只针对这个协程函数, 后续简称为`协程`
    - `挂起`指的是`调用协程`后, 中途保存协程状态, 而后`转移控制权`，此时协程内代码并未执行完
        - `调用协程`不仅指的是第一次调用, 后续的`恢复`操作也是
        - `转移控制权`的行为是协程内部由`co_await`触发的
            - 可以直接转移给调用方
            - 可以通过`await_suspend`返回协程句柄，直接切换到此协程执行，本质是**尾部调用（jmp）**
    - `恢复`指的是调用`协程句柄的resume()`恢复协程的上下文，再次`同线程`执行协程内代码, **本质是一次对协程函数的调用（call）**
        - 执行的代码入口就是`co_await的下一行`
        - `恢复`行为可以在任何线程进行, 需要用户注意资源多线程安全

#### 栈溢出问题
如果有这样的代码: 协程函数A内循环调用协程函数B, 协程函数B需要resume协程函数A, 会导致**栈不断增大直至溢出**
...
- `非对称转换` 协程A `切换到` 协程B时, 做了函数调用
    - 即协程A需要通过resume方法恢复协程B
- `对称转换` 协程A `切换到` 协程B时, 先pop自身栈帧, 然后做函数调用, 也叫`尾部调用`
    - 即协程A通过await_suspend返回句柄方式，直接切换到协程B，避免先回到调度器/调用者再转进目标协程
  ...
  → parent.resume()
    → parent 执行到 co_await child()
      → Awaiter::await_suspend
        → child.resume()           [栈增长]
          → child 执行完毕
        ← 返回到 await_suspend     [栈回退]
      ← 返回到 parent
    ← 返回到 main
  → 需要再次 parent.resume() 继续
```
...
main
  → parent.resume()
    → parent 执行到 co_await child()
      → Awaiter::await_suspend 返回 child 句柄
        → 运行时直接跳转到 child（尾调用）   [栈深度不变]
          → child 执行完毕
          → final_suspend 返回 parent 句柄
        → 运行时直接跳转到 parent            [栈深度不变]
      → parent 继续执行 "parent after"
    ← 一次性完成
```
...
| 特性           | 非对称转换                     | 对称转换                       |
|:---------------|:------------------------------|:------------------------------|
| 返回类型       | `void` 或 `bool`              | `std::coroutine_handle<>`     |
| 栈深度         | 每次转换增加栈帧              | 尾调用优化，栈深度恒定         |
| 恢复方式       | 手动调用 `resume()`           | 运行时自动跳转                 |
| 恢复次数       | 多次手动 resume               | 一次 resume 串联执行           |
| 性能开销       | 栈回退/重入开销               | 接近函数跳转                   |
| 栈溢出风险     | 深层嵌套可能溢出              | O(1) 栈空间                   |
| 实现复杂度     | 简单                          | 需要 continuation 机制         |

### 非对称转换的问题

#### 1. 栈溢出风险
```C++
Task deep_recursion(int n) {
    if (n > 0) {
        co_await deep_recursion(n - 1);  // 非对称：栈不断增长
    }
    co_return;
}
...
#### 协程池调度
```C++
Task schedule_on_thread_pool() {
    co_await thread_pool.schedule();  // 切换线程
    // 对称转换：直接在目标线程恢复，代替调用resume
    perform_work();
    co_return;
}
```
...
### std::noop_coroutine
std::noop_coroutine() 是 C++20 提供的空操作协程句柄，用作协程链的终止标记。

* 返回类型：std::noop_coroutine_handle（可转换为 std::coroutine_handle<>）
* 调用 resume() 时什么都不做，立即返回
* done() 永远返回 false
* 永远不需要 destroy()
* 全局唯一实例
...
作用场景：
1. 作为协程链尾
    1. 当没有调用者时，返回 noop 而非空句柄，避免崩溃。
1. 避免空指针检查
1. 初始化默认值

为什么需要：
* 对称转换中 await_suspend 必须返回有效句柄
* 顶层协程没有调用者，返回 noop 表示"执行完毕，无需继续"
* 避免特殊情况的 if 分支